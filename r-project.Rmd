...
Only `r symmetric*100`% of rides start and end at the same station, which means that `r asymmetric*100`% of rides are asymmetric traffic.
...
`r top10_rides*100`% of bike rides start come from the top 10% most used station. Understanding this, it would be useful to focus on bike availability / station maintenance for the few most frequented stations, as they account for a substantial portion of traffic.
...
```{r}
surplus_stations = station_flow[station_flow$net < 0.5,]
deficit_stations = station_flow[station_flow$net > 1.5,]
```
